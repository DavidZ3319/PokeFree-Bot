  ...
  execute: async (client, message, args, prefix, guild, color, channel) => {
    ...
    if (cmd == "1") {
      ...
      if (args[1].toLowerCase() == "incense") {
        if (user.shards < 351) return message.channel.send("You don't have enough Shards ( `350` ) to buy item **Incense**")
        let msg = await message.channel.send("Please Confirm if you would like to buy `100` **Incense** for `350` **Shards**?")
        await msg.react("✅")
        msg.react("❌")
      ...
      let option = ["oldrod", "newrod","techrod"]
      if (!args[1]) return message.channel.send(`Cmon dude! You don't wanna buy whole SHOP 8.\n\`\`\`\n${prefix}shopbuy 8 <item>\n\`\`\``)
      if (!option.toString().toLowerCase().includes(args[1])) return message.channel.send("Invalid item name provided.")

      if (args[1].toLowerCase() == "oldrod") {
         
      if (user.balance <= 4999) return message.channel.send("You don't have enough balance ( `5000` ) to buy the pokemon **Old Fishing Rod.**")
      if(user.fishingrod === 1) return message.channel.send(">  You Already Own A Fishing Rod In Your Inventory")
     user.balance = user.balance - 5000;
     user.oldfishingrod = user.oldfishingrod +1;
     await user.save();
      return message.channel.send('>  Succesfully bought Fishing Rod. You Can Now Start Fishing and use the p!oldrod command.' )
      }
     ...
     await user.save();
      return message.channel.send('>  Succesfully bought New Fishing Rod. You Can Now Start using the p!newrod command.' ) }
     ...
     await user.save();
      return message.channel.send('>  Succesfully bought New Fishing Rod. You Can Now Start using the p!techrod command. You Luckily Received A Fishing Pass Which Will Increase Your Shiny And Legendary Rate By 40%' ) }
         else {
      return message.channel.send(`Shop Number \`${cmd}\` not found.\n\`\`\`\n${prefix}shopbuy <page> <item> [amount/name]\n\`\`\``)
         } 
       }
    else if (cmd == "9") {
      let option = ["oldriffle", "newriffle","techriffle"]
      if (!args[1]) return message.channel.send(`Cmon dude! You don't wanna buy whole SHOP 8.\n\`\`\`\n${prefix}shopbuy 9 <item>\n\`\`\``)
      if (!option.toString().toLowerCase().includes(args[1])) return message.channel.send("Invalid item name provided.")

      if (args[1].toLowerCase() == "oldriffle") {
         
      if (user.balance <= 4999) return message.channel.send("You don't have enough balance ( `5000` ) to buy the pokemon **Old Hunting Riffle.**")
      if(user.oldhuntingriffle === 1) return message.channel.send(">  You Already Own An Old Hunting Riffle In Your Inventory")
     user.balance = user.balance - 5000;
     user.oldfishingrod = user.oldhuntingriffle +1;
     await user.save();
      return message.channel.send('>  Succesfully bought Old Hunting Riffle. You Can Now Start Hunting and use the p!oldriffle command.' )
      }
      if (args[1].toLowerCase() == "newriffle") {
         
      if (user.balance <= 10000) return message.channel.send("You don't have enough balance ( `1000` ) to buy the pokemon **Old Hunting Riffle.**")
      if(user.newhuntingriffle === 1) return message.channel.send(">  You Already Own An New Hunting Riffle In Your Inventory")
     user.balance = user.balance - 10000;
     user.newfishingrod = user.newfishingrod +1;
     await user.save();
      return message.channel.send('>  Succesfully bought New Hunting Riffle. You Can Now Start Hunting and use the p!newriffle command.' )
      }
      if (args[1].toLowerCase() == "techriffle") {
         
      if (user.balance <= 100000) return message.channel.send("You don't have enough balance ( `100000` ) to buy the pokemon **Old Hunting Riffle.**")
      if(user.techhuntingriffle === 1) return message.channel.send(">  You Already Own An Tech Hunting Riffle In Your Inventory")
     user.balance = user.balance - 100000;
     user.techhuntingriffle = user.techhuntingriffle +1;
     await user.save();
